...
def train_and_evaluate():
    logger.info("🏎️ Step 1: Building anti-leakage historical dataset (2019-2025)...")
    ...
    # 1. Train Calibrated HistGradientBoosting Classifier for Race Win (P1)
    logger.info("🤖 Training Calibrated HistGradientBoosting Classifier for Race Win (P1)...")
    win_base = HistGradientBoostingClassifier(
        max_iter=100,
        max_depth=4,
        learning_rate=0.05,
        random_state=42
    ...
    calibrated_win_model.fit(X_train, y_win_train)

    # 2. Train Calibrated HistGradientBoosting Classifier for Top-10 / Points Finish
    logger.info("🤖 Training Calibrated HistGradientBoosting Classifier for Top-10 / Points Finish...")
    top10_base = HistGradientBoostingClassifier(
        max_iter=100,
        max_depth=4,
        learning_rate=0.05,
        random_state=42
    ...
    calibrated_top10_model.fit(X_train, y_top10_train)

    # 3. Model Calibration & Evaluation on Walk-Forward Test Set
    ...
    artifact = {
        ...
        "algorithm": "Gradient Boosted Trees (HistGradientBoosting + Platt Scaling)",
        "metrics": {
